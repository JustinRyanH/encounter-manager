...
export function AddCharacterToEncounter({
  ...
  const form = useForm<EncounterFormProps>({
    ...
    validate: {
      ...
      initiative: (value) =>
        ActiveCharacter.ValidateInitiative(value || null).join(", ") || null,
      totalHp: (value) =>
        HitPoints.ValidateTotal(value || null).join(", ") || null,
      tempHp: (value) =>
        HitPoints.ValidateTemp(value || null).join(", ") || null,
    },
  });
  ...
  const handleError = () => {
    notifyErrors({
      errors: "some fields are invalid",
