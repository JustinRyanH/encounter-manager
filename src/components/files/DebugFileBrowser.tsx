  ...
  return (
    <Group position="apart">
      ...
      <FileMenu
        file={file}
        opened={menuOpened}
        onClose={menuHandles.close}
        onOpen={menuHandles.open}
      />
    </Group>
  );
}
...
function DirectoryLine({ directory }: { directory: Directory }) {
  ...
  const fileComponents = files.map((file) => {
    if (file.type === "directory")
      return <DirectoryLine key={file.path} directory={file as Directory} />;
  ...
  const fileList = (
    <>
      ...
      >
        <Stack spacing={rem(2)}>{fileComponents}</Stack>
      </Collapse>
    </>
  );
...
export function DebugFileBrowser() {
  ...
  const rootDirectory = useWatchValueObserver(
    fileManager.rootDirectoryObserver
  );
