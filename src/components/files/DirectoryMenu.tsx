...
function RenameFileModal({ file, onClose, opened }: FileModal) {
  ...
  const onSubmit = form.onSubmit((values: { name: string }) =>
    fileManager
      .renameFile(file, values.name)
      .then(() => onClose())
      .catch((e) => {
        notifyErrors({ errors: e.toString() });
        onClose();
...
function CreateDirectoryModal({ directory, onClose, opened }: DirectoryModal) {
  ...
  const onSubmit = form.onSubmit((values) => {
    fileManager
      .touchDirectory(directory, values.name)
      .then(() => onClose())
      .catch((e) => {
        notifyErrors({ errors: e.toString() });
        onClose();
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      size="xs"
      title={`Create directory in "${name}"`}
      centered
    >
      <form onSubmit={onSubmit}>
        <Group noWrap m={rem(4)}>
...
export function DirectoryMenu({
  ...
  return (
    <>
      <CreateFileModal
        directory={directory}
        onClose={createFileHandles.close}
        opened={createFileModal}
      />
      <CreateDirectoryModal
        directory={directory}
        onClose={createDirectoryHandles.close}
        opened={createDirectoryModal}
      />
      <DeleteConfirmationModal
        file={directory}
        onClose={fileDeleteHandles.close}
        opened={fileDeleteModal}
      />
      <RenameFileModal
        file={directory}
        onClose={fileRenameHandles.close}
        opened={fileRenameModal}
      />
      <Menu
        opened={opened}
        onClose={onClose}
      ...
      >
        ...
        <Menu.Dropdown>
          ...
          <Menu.Item
            onClick={fileRenameHandles.open}
            icon={<PencilSimpleLine />}
          >
            Rename Directory
          </Menu.Item>
          <Menu.Item onClick={fileDeleteHandles.open} icon={<TrashSimple />}>
            Delete Directory
          </Menu.Item>
          <Menu.Item
            onClick={createDirectoryHandles.open}
            icon={<FolderSimplePlus />}
          >
            Create Directory
          </Menu.Item>
          <Menu.Divider />
          <Menu.Item onClick={createFileHandles.open} icon={<FilePlus />}>
            Create File
          </Menu.Item>
        </Menu.Dropdown>
      </Menu>
